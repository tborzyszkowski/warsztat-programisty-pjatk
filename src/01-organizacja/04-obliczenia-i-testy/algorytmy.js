function poleProstokata(szerokosc, wysokosc) {
  if (!Number.isFinite(szerokosc) || !Number.isFinite(wysokosc)) {
    throw new TypeError("Boki musza byc liczbami");
  }
...
function srednia(values) {
  if (!Array.isArray(values) || values.length === 0) {
    throw new RangeError("Lista nie moze byc pusta");
  }
  if (values.some((value) => !Number.isFinite(value))) {
    throw new TypeError("Lista musi zawierac liczby");
  }
...
function silnia(number) {
  if (!Number.isInteger(number) || number < 0) {
    throw new RangeError("Silnia wymaga liczby calkowitej nieujemnej");
  }
  let result = 1;
...
function nwd(firstNumber, secondNumber) {
  if (!Number.isInteger(firstNumber) || !Number.isInteger(secondNumber)) {
    throw new TypeError("NWD wymaga liczb calkowitych");
  }
  let first = Math.abs(firstNumber);
  let second = Math.abs(secondNumber);
  if (first === 0 && second === 0) {
    throw new RangeError("NWD dla dwoch zer nie jest okreslone");
  }
  while (second !== 0) {
